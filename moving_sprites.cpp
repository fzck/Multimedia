...
void plotLineBlack(int x0, int y0, int x1, int y1)
{
...
void plotLine(int x0, int y0, int x1, int y1)
{
   ...
   for(;;){  /* loop */
      SetPixel(mydc, x0,y0, RGB(0,0,255));
      if (x0==x1 && y0==y1) break;
      e2 = 2*err;
      if (e2 >= dy) { err += dy; x0 += sx; } /* e_xy+e_x > 0 */
      if (e2 <= dx) { err += dx; y0 += sy; } /* e_xy+e_y < 0 */
...
{ /* plot a limited quadratic Bezier segment */
  ...
  if (cur != 0) { /* no straight line */
    ...
    do {
      SetPixel(mydc, x0,y0,RGB(0,0,255)); /* plot curve */
...
void plotQuadBezierSegBlack(int x0, int y0, int x1, int y1, int x2, int y2)
{ /* plot a limited quadratic Bezier segment */
  int sx = x2-x1, sy = y2-y1;
  long xx = x0-x1, yy = y0-y1, xy; /* relative values for checks */
  double dx, dy, err, cur = xx*sy-yy*sx; /* curvature */
  ...
  if (cur != 0) { /* no straight line */
    ...
    xx += xx; yy += yy; err = dx+dy+xy; /* error 1st step */
    do {
      SetPixel(mydc, x0,y0,RGB(0,0,0)); /* plot curve */
      if (x0 == x2 && y0 == y2) return; /* last pixel -> curve finished */
      y1 = 2*err < dx; /* save value for test of y step */
      if (2*err > dy) { x0 += sx; dx -= xy; err += dy += yy; } /* x step */
      if ( y1 ) { y0 += sy; dy -= xy; err += dx += xx; } /* y step */
...
void plotCubicBezierSegBlack(int x0, int y0, float x1, float y1,
                        float x2, float y2, int x3, int y3)
{                                        /* plot limited cubic Bezier segment */
   int f, fx, fy, leg = 1;
   int sx = x0 < x3 ? 1 : -1, sy = y0 < y3 ? 1 : -1;        /* step direction */
   ...
   if (xa == 0 && ya == 0) {                              /* quadratic Bezier */
      sx = floor((3*x1-x0+1)/2); sy = floor((3*y1-y0+1)/2);   /* new midpoint */
      return plotQuadBezierSegBlack(x0,y0, sx,sy, x3,y3);
   }
   x1 = (x1-x0)*(x1-x0)+(y1-y0)*(y1-y0)+1;                    /* line lengths */
   x2 = (x2-x3)*(x2-x3)+(y2-y3)*(y2-y3)+1;
   do {                                                /* loop over both ends */
...
exit: xx = x0; x0 = x3; x3 = xx; sx = -sx; xb = -xb;             /* swap legs */
   ...
   plotLineBlack(x0,y0, x3,y3);       /* remaining part in case of cusp or crunode */
}
...
void cubicCurveToBlack(int x0, int y0, int x1, int y1,
                     int x2, int y2, int x3, int y3)
{                                              /* plot any cubic Bezier curve */
   int n = 0, i = 0;
   long xc = x0+x1-x2-x3, xa = xc-4*(x1-x2);
   ...
   for (i = 0; i <= n; i++) {                 /* plot each segment separately */
      ...
      if (fy0 != 0.0) { fy1 *= fy0 = (y0-y3)/fy0; fy2 *= fy0; }
      if (x0 != x3 || y0 != y3)                            /* segment t1 - t2 */
         plotCubicBezierSegBlack(x0,y0, x0+fx1,y0+fy1, x0+fx2,y0+fy2, x3,y3);
      x0 = x3; y0 = y3; fx0 = fx3; fy0 = fy3; t1 = t2;
   }
}
...
void drawWaterBlack(int x, int y){
    /*
            int xleft=x/2;
            int xright =x * 1.5;
            int yleft = y/2;
        ...
        */
            ...
            int yleft = y+50;
            int yright = y+50;
         cubicCurveToBlack(x,y,xleft,yleft,xright,yright,x,y);
...
int main(){
    ...
    /*
    int x0,y0,x1, y1, x2, y2, x3,y3, r1, r2;
    cout << "x0: ";
    cin >> x0;
    cout << endl;
    cout << "y0: ";
    ...
    */
    ...
    while(true){
        ...
        /*
        for(int i = 15; i < 50; i++ ){
            drawFireElement(i,100);
        }
        */

        //drawWaterElement(30,150);
        //plotQuadBezier(0,0,50,0,100,100);
        //plotCubicBezier(0,0,50,100,0,170,250,0);
        //cubicCurveTo(x0, y0, x1, y1,
        //              x2, y2, x3, y3);
        int i;
        for (i = 50; i < 300; i++){
                drawWater(120,i);
                drawWaterBlack(120,i-1);

                drawWater(220,i);
                drawWaterBlack(220,i-1);
        ...
        drawWater(220,i);
         ...
         //cubicCurveTo(300,100, 250,150, 350,150, 300,100);
